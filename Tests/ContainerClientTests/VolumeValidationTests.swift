...
struct VolumeValidationTests {
    ...
    func testVolumeNameEdgeCases() {
        ...
        // Test Docker compatibility - these should follow Docker's rules
    ...
    @Test("Common Docker volume name patterns")
    func testDockerLikeVolumeNames() {
        let dockerPatterns = [
        ...
        for name in dockerPatterns {
            #expect(VolumeStorage.isValidVolumeName(name), "Docker pattern '\(name)' should be valid")
        }
    }
}
